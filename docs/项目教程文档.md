...
这是一个面向航空公司/机场运营团队的 Agentic BI 与运营诊断系统，场景是航班运行、天气影响、异常发现和行业基准对比。

它不是给普通乘客查实时航班或买机票的助手，而是服务运营复盘和决策支持：基于历史航班-天气数据回答“哪里异常、为什么异常、和基准相比如何、下一步怎么优化”。
...
- Skill Registry：用轻量 YAML 注册工具/技能边界，包括 SQLTool、SearchTool、ChartTool、AnomalySkill、WeatherImpactSkill、ReportSkill
...
```bash
export DASHSCOPE_API_KEY="你的模型Key"
export TAVILY_API_KEY="你的搜索Key"  # 可选，不配也能跑 SQL 路径
```
